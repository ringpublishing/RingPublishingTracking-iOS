...
class DecoratorTests: XCTestCase {
    ...
    func testParameters_clientDecoratorViewTypeUpdated_returnedParametersAreCorrect() {
        // Given
        let decorator = ClientDecorator()

        // When
        decorator.updateViewType(.text)

        // Then
        XCTAssertEqual(decorator.parameters["RDLC"],
                       "eyJjbGllbnQiOnsidHlwZSI6Im5hdGl2ZV9hcHAiLCJ2aWV3VHlwZSI6InRleHQifX0=",
                       "RDLC should contain text view type")

        // When
        decorator.updateViewType(.video)
        ...
        // When
        decorator.updateViewType(.audio)
    ...
    func testParameters_clientDecoratorViewTypeCleared_returnedParametersAreCorrect() {
        // Given
        let decorator = ClientDecorator()
        decorator.updateViewType(.audio)

        // When
        decorator.updateViewType(nil)

        // Then
        XCTAssertEqual(decorator.parameters["RDLC"],
                       "eyJjbGllbnQiOnsidHlwZSI6Im5hdGl2ZV9hcHAifX0=",
                       "RDLC should not contain view type")
    }
}
